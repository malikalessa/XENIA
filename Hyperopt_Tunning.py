...
def NN(params):
    ...
    if gc.config_No == 99 : # for baseline fine tuning using XAI
        model = load_model(gc.baseline_tobe_tunned )

    if gc.config_No == 111 :  # for double fine tuning baseline
        model = load_model(path +'XAI_fine_tuning_baseline.h5' )

    if gc.config_No==22: # for Conf 6 fine tuning using XAI
        model = load_model(path + 'ConfigNo6Attack_Type_1_NN.h5')

    if gc.config_No == 55: # for Conf 6 double fine tuning
        model = load_model(path+'XAI_fine_tuning_Conf6.h5')

    adam = Adam(learning_rate=params['learning_rate'])
    model.trainable = True
    for l in model.layers:
        print(l.name, l.trainable)
        if (l.name == "batch_normalization") :
            l.trainable = False
            print(l.name, l.trainable)
...
def hypersearch(train_scaler, y_train, test_scaler, y_test, testPath, config_No):
    ...
    gc.model_path = testPath
    gc.config_No = config_No
    ...
    # gc.best_model.save_weights(best_weights)
